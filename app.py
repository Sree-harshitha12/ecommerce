...
import os
import razorpay
RAZORPAY_KEY_ID='rzp_test_k8hXmsmZPmrdpJ'
RAZORPAY_KEY_SECRET='Oculeea10ZYdz3H9Nti57LYa'
...
def category(category):
    if session.get('user'):
        ...
        return redirect( url_for('login'))
@app.route('/pay/<itemid>/<name>/<float:price>', methods=['POST'])
def pay(itemid, name, price):
    try:
        ...
        total_price = int(price) * qty * 100  # Ensure integer multiplication
